...
<?php
	...
	# Check the Nitrogen form is submitted
	if (isset($_POST['next'])) {
		...
		# Check if pixcolor exist in table nitrogen, insert if not.
		$hex_val = substr($_POST["pixcolor"], 1);
		$level = 'low'; # Always low

		$sql = "SELECT * from tblnitrogen where hex_val = '{$hex_val}'";
		$result = $conn->query($sql);

		if (!$row = $result->fetch_assoc()) {
			# The hex_val value does not exist, create a new entry.
			$sql = "INSERT INTO tblnitrogen (hex_val, level) VALUES ('{$hex_val}', '{$level}')";
			$conn->query($sql);
		}

		# Update sample's res_n and res_n_val
		$sql = "UPDATE temp SET res_n = '{$level}', res_n_val = '{$hex_val}' WHERE id = {$id}";

		if ($conn->query($sql) === TRUE) {
			echo "<script type= 'text/javascript'>alert('New record created successfully'); window.location.href='phosporus.php?id={$id}'; </script>";
		} else {
	...
	<div class="media-container-row">
		<div class="title col-12 col-md-8">
			<h2 class="align-center pb-3 mbr-fonts-style display-2"><br><br>
				<?php echo "Nitrogen {$sample_row['sample']}"; ?>
			</h2>
